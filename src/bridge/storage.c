...
int storage_create_snapshot(char** snapshot_data, size_t* snapshot_size) {
    if (!snapshot_data || !snapshot_size) return -1;
...
int storage_restore_snapshot(const char* snapshot_data, size_t snapshot_size) {
    if (!snapshot_data || snapshot_size == 0) return -1;
    ...
    uint32_t version;
    uint64_t timestamp;
    if (ptr + sizeof(version) + sizeof(timestamp) > snapshot_data + snapshot_size) return -1;
    ...
    if (ptr + 5 * sizeof(size_t) > snapshot_data + snapshot_size) return -1;
    ...
    // 验证数据完整性
    if (ptr + array_size + hash_size + rbtree_size + btree_size + skiplist_size > snapshot_data + snapshot_size) {
        return -1;
    }
