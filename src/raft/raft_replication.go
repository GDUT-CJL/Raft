...
func (rf *Raft)AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply){
	...
	if args.PrevLogIndex >= len(rf.log){
		...
		LOG(rf.me,rf.currentTerm,DLog2,"<- S%d,Reject log,Follower too short,len : %d <= Pre:%d",args.LeaderId,len(rf.log),args.PrevLogIndex)
		return
	}
	...
	if rf.log[args.PrevLogIndex].Term != args.PrevLogTerm{
		...
		reply.ConfilictIndex = rf.firstLogFor(reply.ConfilictTerm) // 找到这个任期的第一个日志
		LOG(rf.me,rf.currentTerm,DLog2,"<- S%d,Reject log,Pre Log not match,[%d]: T%d != T%d",args.LeaderId,args.PrevLogTerm,rf.log[args.PrevLogIndex].Term)
		return
	}
	...
	if args.LeaderCommit > rf.commitIndex {
		...
		if rf.commitIndex >= len(rf.log) {// 如果本地提交日志号比本身日志条目还大
			rf.commitIndex = len(rf.log) - 1 // 则将要提交的日志号更新为日志长度 - 1
		}
...
func (rf *Raft)startReplication(term int) bool{
	replicateToPeer:=func(peer int,args *AppendEntriesArgs){
		...
		if majorityMactched > rf.commitIndex && rf.log[majorityMactched].Term == rf.currentTerm{
	...
	for peer:=0;peer < len(rf.peers);peer++{
		...
		args:= &AppendEntriesArgs{
			...
			Entries:rf.log[prevIndex+1:], // 从 rf.log 切片中截取从索引 prevIndex+1 开始到末尾的所有日志条目。左闭右开
